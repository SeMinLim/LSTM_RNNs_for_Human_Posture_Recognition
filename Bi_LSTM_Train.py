...
class Config(object):
    def __init__(self, X_train, X_test):
        ...
        self.batch_size = 5
        self.save_file = './train_model.ckpt'
        ...
        self.W = {
            ...
            'output': tf.Variable(tf.random_normal([2*self.n_hidden, self.n_classes])) # Bidirectional
        }
        self.biases = {
...
def LSTM_Network(_X, config):
    ...
    back_lstm_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell_1, lstm_cell_2], state_is_tuple=True)
    forward_lstm_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell_1, lstm_cell_2], state_is_tuple=True)
    # Get LSTM cell output
    try:
        outputs, _, _ = tf.contrib.rnn.static_bidirectional_rnn(forward_lstm_cell, back_lstm_cell, _X, dtype=tf.float32)
    except Exception:
        outputs = tf.contrib.rnn.static_bidirectional_rnn(forward_lstm_cell, back_lstm_cell, _X, dtype=tf.float32)
